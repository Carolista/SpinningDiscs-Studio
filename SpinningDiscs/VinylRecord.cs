...
public class VinylRecord : Media, IVariableRPM
{
    ...
    public VinylRecord(string name, int diameter)
        : base(name, "record", 0, 440)
    {
        Diameter = diameter;
        CalculateSpinRate();
    }
    ...
    {
        return base.ToString() + GetFileList("Tracks");
    }
    ...
    public void PressVinyl(MediaFile[] fileArr)
    {
        ...
        {
            Console.WriteLine(
                nl + "There is not enough space for all the files to be pressed to vinyl."
            );
        }
    }
